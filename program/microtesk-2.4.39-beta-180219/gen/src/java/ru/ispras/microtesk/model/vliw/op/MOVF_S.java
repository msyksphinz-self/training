...
public final class MOVF_S extends IsaPrimitive {
  ...
  public MOVF_S(final F fd, final F fs, final Immediate cc) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("MOVF_S %s, %s, %d", fd.text(vars__), fs.text(vars__), cc.access().load().bigIntegerValue());
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s00%s%s010001", cc.access().load().toString(), fs.image(vars__), fd.image(vars__));
  }
