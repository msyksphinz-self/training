...
public final class lw extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new lw(rt, offset, address);
    }
  }
  ...
  public lw(final REG rt, final Immediate offset, final REG address) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("lw %s, %d(%s)", rt.text(vars__), offset.access().load().bigIntegerValue(), address.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("100011%s%s%s", address.image(vars__), rt.image(vars__), offset.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rt.access(pe__, vars__).store(pe__.M.access(address.access(pe__, vars__).load().add(Data.signExtend(WORD, offset.access().load())).shiftRight(Data.valueOf(WORD, 0x2))).load());
  }
}