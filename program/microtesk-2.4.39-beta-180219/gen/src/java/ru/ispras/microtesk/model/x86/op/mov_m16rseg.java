...
public final class mov_m16rseg extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new mov_m16rseg(seg_ovrd, dst, src);
    }
  }
  ...
  public mov_m16rseg(final RSEG16 seg_ovrd, final IsaPrimitive dst, final RSEG16 src) {
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11010001100%s0%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), src.image(vars__), vars__.rm.access().load().toString(), dst.image(vars__));
  }
