...
public final class jalr extends IsaPrimitive {
  ...
  public jalr(final REG rd, final REG rs) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("jalr %s, %s", rd.text(vars__), rs.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("000000%s00000%s00000001001", rs.image(vars__), rd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rd.access(pe__, vars__).store(pe__.CIA.access().load().add(Data.valueOf(WORD, 0x4)).add(Data.valueOf(WORD, 0x4)));
    pe__.BRANCH.access().store(Data.valueOf(BIT, 0x1));
    pe__.JMPADDR.access().store(rs.access(pe__, vars__).load());
  }
}