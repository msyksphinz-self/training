...
public final class or extends IsaPrimitive {
  ...
  public or(final REG rd, final REG rs, final REG rt) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("or %s, %s, %s", rd.text(vars__), rs.text(vars__), rt.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("000000%s%s%s00000100101", rs.image(vars__), rt.image(vars__), rd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rd.access(pe__, vars__).store(rs.access(pe__, vars__).load().or(rt.access(pe__, vars__).load()));
  }
}