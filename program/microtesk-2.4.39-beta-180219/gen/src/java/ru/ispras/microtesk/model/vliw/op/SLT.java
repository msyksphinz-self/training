...
public final class SLT extends IsaPrimitive {
  ...
  public SLT(final R rd, final R rs, final R rt) {
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("000000%s%s%5s00000101010", rs.image(vars__), rt.image(vars__), rd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rd.access(pe__, vars__).store(Data.valueOf(WORD, ((rs.access(pe__, vars__).load().compareTo(rt.access(pe__, vars__).load()) < 0) ? 1 : 0)));
  }
}