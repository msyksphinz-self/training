...
public final class IAM_SI_D8 extends IsaPrimitive {
  public static final String NAME = "IAM_SI_D8";
  ...
  public IAM_SI_D8(final Immediate disp) {
    this.disp = disp;
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    init(vars__);
    return String.format("%d[SI]", disp.access().load().bigIntegerValue());
  }
  ...
  public void init(final TemporaryVariables tempVars) {
    ...
    vars__.rm.access().store(Data.valueOf(Type.CARD(3), 0x4));
  }
  ...
  public Location access(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    final PE pe__ = (PE) procElem;
    final TempVars vars__ = (TempVars) tempVars;
    return annotate(new Location(Data.signExtend(addR, Data.cast(SWORD, Data.signExtend(WORD, disp.access().load()).add(pe__.SI.access().load())))), vars__);
  }
}