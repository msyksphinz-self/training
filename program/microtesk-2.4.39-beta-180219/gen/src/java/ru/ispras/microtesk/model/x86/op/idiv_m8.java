...
public final class idiv_m8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new idiv_m8(seg_ovrd, op1);
    }
  }
  ...
  public idiv_m8(final RSEG16 seg_ovrd, final IsaPrimitive op1) {
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    vars__.tmpw_op1.access().store(Data.cast(SWORD, pe__.AX.access().load()).divide(Data.signExtend(SWORD, Data.cast(SBYTE, pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load())).load()))));
    ...
    if ((vars__.tmpw_op1.access().load().compareTo(Data.valueOf(WORD, 0x7f)) <= 0) && (vars__.tmpw_op1.access().load().compareTo(Data.valueOf(WORD, 0x80)) >= 0)) {
      pe__.AH.access().store(Data.coerce(BYTE, pe__.AX.access().load().mod(Data.signExtend(WORD, Data.cast(SBYTE, pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load())).load())))));
