...
public final class C_EQ_S extends IsaPrimitive {
  ...
  public C_EQ_S(final F ft, final F fs, final Immediate cc) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("C_EQ_S %d, %s, %s", cc.access().load().bigIntegerValue(), fs.text(vars__), ft.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s%s%s00110010", ft.image(vars__), fs.image(vars__), cc.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    if (fs.access(pe__, vars__).load().equals(ft.access(pe__, vars__).load())) {
