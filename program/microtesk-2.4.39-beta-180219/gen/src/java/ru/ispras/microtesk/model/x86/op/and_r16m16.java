...
public final class and_r16m16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new and_r16m16(seg_ovrd, op1, op2);
    }
  }
  ...
  public and_r16m16(final RSEG16 seg_ovrd, final R16 op1, final IsaPrimitive op2) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("and %s, %s:%s", op1.text(vars__), seg_ovrd.text(vars__), op2.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11000100011%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), op1.image(vars__), vars__.rm.access().load().toString(), op2.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new test_proc_word(new Immediate(Location.concat(pe__.MEM.access(vars__.base.access().load().add(op2.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))), pe__.MEM.access(vars__.base.access().load().add(op2.access(pe__, vars__).load())))), new Immediate(op1.access(pe__, vars__).load().add(Data.valueOf(WORD, 0x0)))).execute(pe__, vars__);
    pe__.MEM.access(vars__.base.access().load().add(op2.access(pe__, vars__).load())).store(vars__.tmpw_res.access().bitField(7, 0).load());
    pe__.MEM.access(vars__.base.access().load().add(op2.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))).store(vars__.tmpw_res.access().bitField(15, 8).load());
