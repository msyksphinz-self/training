...
public final class add_m8r8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new add_m8r8(seg_ovrd, op1, op2);
    }
  }
  ...
  public add_m8r8(final RSEG16 seg_ovrd, final IsaPrimitive op1, final R8 op2) {
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11000000000%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), op2.image(vars__), vars__.rm.access().load().toString(), op1.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new add_proc_byte(new Immediate(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()))), new Immediate(op2.access(pe__, vars__).load().add(Data.valueOf(Type.CARD(8), 0x0)))).execute(pe__, vars__);
