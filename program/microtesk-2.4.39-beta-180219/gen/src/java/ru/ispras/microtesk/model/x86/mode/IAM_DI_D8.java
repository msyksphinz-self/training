...
public final class IAM_DI_D8 extends IsaPrimitive {
  public static final String NAME = "IAM_DI_D8";
  ...
  public IAM_DI_D8(final Immediate disp) {
    this.disp = disp;
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    init(vars__);
    return String.format("%d[DI]", disp.access().load().bigIntegerValue());
  }
  ...
  public void init(final TemporaryVariables tempVars) {
    ...
    vars__.rm.access().store(Data.valueOf(Type.CARD(3), 0x5));
  }
  ...
  public Location access(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    final PE pe__ = (PE) procElem;
    final TempVars vars__ = (TempVars) tempVars;
    return annotate(new Location(Data.signExtend(addR, Data.cast(SWORD, Data.signExtend(WORD, disp.access().load()).add(pe__.DI.access().load())))), vars__);
  }
}