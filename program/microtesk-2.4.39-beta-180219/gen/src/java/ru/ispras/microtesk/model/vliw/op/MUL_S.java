...
public final class MUL_S extends IsaPrimitive {
  ...
  public MUL_S(final F fd, final F fs, final F ft) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("MUL_S %s, %s, %s", fd.text(vars__), fs.text(vars__), ft.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s%s%s000010", ft.image(vars__), fs.image(vars__), fd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    fd.access(pe__, vars__).store(fs.access(pe__, vars__).load().multiply(ft.access(pe__, vars__).load()));
  }
}