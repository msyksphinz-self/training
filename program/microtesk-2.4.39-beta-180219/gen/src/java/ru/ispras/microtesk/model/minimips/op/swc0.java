...
public final class swc0 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new swc0(cs, offset, address);
    }
  }
  ...
  public swc0(final RCOP0 cs, final Immediate offset, final REG address) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("swc0 %s, %d(%s)", cs.text(vars__), offset.access().load().bigIntegerValue(), address.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("111000%s%s%s", address.image(vars__), cs.image(vars__), offset.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    pe__.M.access(address.access(pe__, vars__).load().add(Data.signExtend(WORD, offset.access().load())).shiftRight(Data.valueOf(WORD, 0x2))).store(cs.access(pe__, vars__).load());
  }
}