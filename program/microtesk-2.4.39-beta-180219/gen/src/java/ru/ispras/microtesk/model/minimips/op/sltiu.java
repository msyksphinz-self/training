...
public final class sltiu extends IsaPrimitive {
  ...
  public sltiu(final REG rt, final REG rs, final Immediate imm) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("sltiu %s, %s, 0x%x", rt.text(vars__), rs.text(vars__), imm.access().load().bigIntegerValue(false));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("001011%s%s%s", rs.image(vars__), rt.image(vars__), imm.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    if ((Data.cast(Type.CARD(32), rs.access(pe__, vars__).load()).compareTo(Data.signExtend(Type.CARD(32), imm.access().load())) < 0)) {
      rt.access(pe__, vars__).store(Data.valueOf(WORD, 0x1));
    } else {
      rt.access(pe__, vars__).store(Data.valueOf(WORD, 0x0));
    }
  }
}