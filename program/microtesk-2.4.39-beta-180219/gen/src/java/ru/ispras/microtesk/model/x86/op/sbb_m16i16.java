...
public final class sbb_m16i16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new sbb_m16i16(seg_ovrd, op1, op2);
    }
  }
  ...
  public sbb_m16i16(final RSEG16 seg_ovrd, final IsaPrimitive op1, final IMM16 op2) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("sbb %s:%s, %s", seg_ovrd.text(vars__), op1.text(vars__), op2.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11010000001%s000%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), vars__.rm.access().load().toString(), op1.image(vars__), op2.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new sub_proc_word(new Immediate(Location.concat(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))), pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load())))), new Immediate(op2.access(pe__, vars__).load().subtract(Data.zeroExtend(WORD, pe__.CF.access().load())))).execute(pe__, vars__);
    Location.concat(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load())), pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1)))).store(Location.concat(vars__.tmpw_res.access().bitField(7, 0), vars__.tmpw_res.access().bitField(15, 8)));
