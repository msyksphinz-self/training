...
public final class mul_m16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new mul_m16(seg_ovrd, op1);
    }
  }
  ...
  public mul_m16(final RSEG16 seg_ovrd, final IsaPrimitive op1) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("mul %s:%s", seg_ovrd.text(vars__), op1.text(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    vars__.tmpdw_op1.access().store(Data.zeroExtend(DWORD, pe__.AX.access().load()).multiply(Data.zeroExtend(DWORD, Location.concat(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))), pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()))).load())));
