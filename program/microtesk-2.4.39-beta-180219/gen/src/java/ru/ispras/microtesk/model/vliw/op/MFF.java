...
public final class MFF extends IsaPrimitive {
  ...
  public MFF(final R rt, final F fs) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("MFF %s, %s", rt.text(vars__), fs.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000100000%s%s00000000000", rt.image(vars__), fs.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rt.access(pe__, vars__).store(fs.access(pe__, vars__).load());
  }
}