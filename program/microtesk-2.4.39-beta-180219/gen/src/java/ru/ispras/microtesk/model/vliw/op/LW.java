...
public final class LW extends IsaPrimitive {
  ...
  public LW(final R base, final R rt, final Immediate offset) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("LW %s, %d(%s)", rt.text(vars__), offset.access().load().bigIntegerValue(), base.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("100011%s%s%s", base.image(vars__), rt.image(vars__), offset.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rt.access(pe__, vars__).store(pe__.M.access(base.access(pe__, vars__).load().add(Data.signExtend(WORD, offset.access().load())).shiftRight(Data.valueOf(WORD, 0x2))).load());
  }
}