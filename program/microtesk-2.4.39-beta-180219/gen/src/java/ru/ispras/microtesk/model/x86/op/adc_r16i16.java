...
public final class adc_r16i16 extends IsaPrimitive {
  ...
  public adc_r16i16(final R16 op1, final IMM16 op2) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("adc %s, %s", op1.text(vars__), op2.text(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    new add_proc_word(new Immediate(op1.access(pe__, vars__).load().add(Data.valueOf(WORD, 0x0))), new Immediate(op2.access(pe__, vars__).load().add(Data.zeroExtend(WORD, pe__.CF.access().load())))).execute(pe__, vars__);
