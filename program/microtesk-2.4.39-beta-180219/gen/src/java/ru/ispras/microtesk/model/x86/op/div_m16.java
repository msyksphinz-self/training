...
public final class div_m16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new div_m16(seg_ovrd, op1);
    }
  }
  ...
  public div_m16(final RSEG16 seg_ovrd, final IsaPrimitive op1) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("div %s:%s", seg_ovrd.text(vars__), op1.text(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    vars__.tmpdw_op2.access().store(vars__.tmpdw_op1.access().load().divide(Data.zeroExtend(DWORD, vars__.tmpw_op1.access().load())));
    vars__.instruction_image_size.access().store(Data.valueOf(SIZE, 0x3));
    op1.execute(pe__, vars__);
    if ((vars__.tmpdw_op1.access().load().compareTo(Data.valueOf(DWORD, 0xffff)) <= 0)) {
      pe__.AX.access().store(Data.coerce(WORD, vars__.tmpdw_op2.access().load()));
      pe__.DX.access().store(Data.coerce(WORD, vars__.tmpdw_op1.access().load().mod(Data.zeroExtend(DWORD, vars__.tmpw_op1.access().load()))));
    }
  }
}