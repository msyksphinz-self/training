...
public final class sltu extends IsaPrimitive {
  ...
  public sltu(final REG rd, final REG rs, final REG rt) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("sltu %s, %s, %s", rd.text(vars__), rs.text(vars__), rt.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("000000%s%s%s00000101011", rs.image(vars__), rt.image(vars__), rd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    if ((Data.cast(Type.CARD(32), rs.access(pe__, vars__).load()).compareTo(Data.cast(Type.CARD(32), rt.access(pe__, vars__).load())) < 0)) {
