...
public final class xchg_r16r16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new xchg_r16r16(dst, src);
    }
  }
  ...
  public xchg_r16r16(final R16 dst, final R16 src) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("xchg %s, %s", dst.text(vars__), src.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("1000011111%s%s", dst.image(vars__), src.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    dst.access(pe__, vars__).store(dst.access(pe__, vars__).load().xor(src.access(pe__, vars__).load()));
    src.access(pe__, vars__).store(src.access(pe__, vars__).load().xor(dst.access(pe__, vars__).load()));
    dst.access(pe__, vars__).store(dst.access(pe__, vars__).load().xor(src.access(pe__, vars__).load()));
