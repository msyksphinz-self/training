...
public final class push_m16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new push_m16(seg_ovrd, src);
    }
  }
  ...
  public push_m16(final RSEG16 seg_ovrd, final IsaPrimitive src) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("push %s:%s", seg_ovrd.text(vars__), src.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11011111111%s000%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), vars__.rm.access().load().toString(), src.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    pe__.MEM.access(Data.zeroExtend(addR, pe__.SS.access().load()).shiftLeft(Data.valueOf(addR, 0x4)).add(Data.signExtend(addR, Data.cast(SWORD, pe__.SP.access().load())))).store(pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load())).load());
    pe__.MEM.access(Data.zeroExtend(addR, pe__.SS.access().load()).shiftLeft(Data.valueOf(addR, 0x4)).add(Data.signExtend(addR, Data.cast(SWORD, pe__.SP.access().load().add(Data.valueOf(WORD, 0x1)))))).store(pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))).load());
