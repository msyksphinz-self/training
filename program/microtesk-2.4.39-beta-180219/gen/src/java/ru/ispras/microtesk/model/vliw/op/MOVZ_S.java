...
public final class MOVZ_S extends IsaPrimitive {
  ...
  public MOVZ_S(final F fd, final F fs, final R rt) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("MOVZ_S %s, %s, %s", fd.text(vars__), fs.text(vars__), rt.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s%s%s010010", rt.image(vars__), fs.image(vars__), fd.image(vars__));
  }
