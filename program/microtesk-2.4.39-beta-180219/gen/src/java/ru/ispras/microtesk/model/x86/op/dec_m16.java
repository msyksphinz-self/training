...
public final class dec_m16 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new dec_m16(seg_ovrd, op1);
    }
  }
  ...
  public dec_m16(final RSEG16 seg_ovrd, final IsaPrimitive op1) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("dec %s:%s", seg_ovrd.text(vars__), op1.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11011111111%s001%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), vars__.rm.access().load().toString(), op1.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new dec_proc_word(new Immediate(Location.concat(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()).add(Data.valueOf(addR, 0x1))), pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()))))).execute(pe__, vars__);
