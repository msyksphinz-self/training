...
public final class ml extends IsaPrimitive {
  ...
  public ml(final REG rt, final Immediate im) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("ml %s %x", rt.text(vars__), im.access().load().bigIntegerValue(false));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("0001%s%s", rt.image(vars__), im.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rt.access(pe__, vars__).bitField(7, 0).store(im.access().load());
  }
}