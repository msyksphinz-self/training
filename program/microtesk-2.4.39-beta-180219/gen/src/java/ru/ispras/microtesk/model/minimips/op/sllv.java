...
public final class sllv extends IsaPrimitive {
  ...
  public sllv(final REG rd, final REG rt, final REG rs) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("sllv %s, %s, %s", rd.text(vars__), rt.text(vars__), rs.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("00000000000%s%s%s000100", rt.image(vars__), rd.image(vars__), rs.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rd.access(pe__, vars__).store(rt.access(pe__, vars__).load().shiftLeft(rs.access(pe__, vars__).bitField(4, 0).load()));
  }
}