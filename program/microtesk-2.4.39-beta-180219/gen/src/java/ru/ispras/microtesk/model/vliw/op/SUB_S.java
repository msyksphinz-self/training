...
public final class SUB_S extends IsaPrimitive {
  ...
  public SUB_S(final F fd, final F fs, final F ft) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("SUB_S %s, %s, %s", fd.text(vars__), fs.text(vars__), ft.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s%s%s000001", ft.image(vars__), fs.image(vars__), fd.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    fd.access(pe__, vars__).store(fs.access(pe__, vars__).load().subtract(ft.access(pe__, vars__).load()));
  }
}