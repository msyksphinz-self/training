...
public final class C_LT_S extends IsaPrimitive {
  ...
  public C_LT_S(final F ft, final F fs, final Immediate cc) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("C_LT_S %d, %s, %s", cc.access().load().bigIntegerValue(), fs.text(vars__), ft.text(vars__));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("01000110000%s%s%s00111100", ft.image(vars__), fs.image(vars__), cc.access().load().toString());
  }
