...
public final class test_m8r8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new test_m8r8(seg_ovrd, op1, op2);
    }
  }
  ...
  public test_m8r8(final RSEG16 seg_ovrd, final IsaPrimitive op1, final R8 op2) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("test %s:%s, %s", seg_ovrd.text(vars__), op1.text(vars__), op2.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11010000100%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), op2.image(vars__), vars__.rm.access().load().toString(), op1.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new test_proc_byte(new Immediate(pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load()))), new Immediate(op2.access(pe__, vars__).load().add(Data.valueOf(Type.CARD(8), 0x0)))).execute(pe__, vars__);
