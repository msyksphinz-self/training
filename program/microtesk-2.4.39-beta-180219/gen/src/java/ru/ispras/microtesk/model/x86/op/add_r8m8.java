...
public final class add_r8m8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new add_r8m8(seg_ovrd, op1, op2);
    }
  }
  ...
  public add_r8m8(final RSEG16 seg_ovrd, final R8 op1, final IsaPrimitive op2) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("add %s, %s:%s", op1.text(vars__), seg_ovrd.text(vars__), op2.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11000000010%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), op1.image(vars__), vars__.rm.access().load().toString(), op2.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    new add_proc_byte(new Immediate(op1.access(pe__, vars__).load().add(Data.valueOf(Type.CARD(8), 0x0))), new Immediate(pe__.MEM.access(vars__.base.access().load().add(op2.access(pe__, vars__).load())))).execute(pe__, vars__);
