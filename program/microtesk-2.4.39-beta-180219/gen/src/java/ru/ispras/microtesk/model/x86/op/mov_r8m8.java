...
public final class mov_r8m8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new mov_r8m8(seg_ovrd, dst, src);
    }
  }
  ...
  public mov_r8m8(final RSEG16 seg_ovrd, final R8 dst, final IsaPrimitive src) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("mov %s, %s:%s", dst.text(vars__), seg_ovrd.text(vars__), src.text(vars__));
  }
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11010001010%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), dst.image(vars__), vars__.rm.access().load().toString(), src.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    dst.access(pe__, vars__).store(pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load())).load());
