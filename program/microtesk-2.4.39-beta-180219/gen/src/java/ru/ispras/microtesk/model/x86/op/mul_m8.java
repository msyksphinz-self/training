...
public final class mul_m8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new mul_m8(seg_ovrd, op1);
    }
  }
  ...
  public mul_m8(final RSEG16 seg_ovrd, final IsaPrimitive op1) {
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    pe__.AX.access().store(Data.zeroExtend(WORD, pe__.AL.access().load()).multiply(Data.zeroExtend(WORD, pe__.MEM.access(vars__.base.access().load().add(op1.access(pe__, vars__).load())).load())));
    vars__.instruction_image_size.access().store(Data.valueOf(SIZE, 0x3));
    op1.execute(pe__, vars__);
    if (!pe__.AH.access().load().equals(Data.valueOf(BYTE, 0x0))) {
