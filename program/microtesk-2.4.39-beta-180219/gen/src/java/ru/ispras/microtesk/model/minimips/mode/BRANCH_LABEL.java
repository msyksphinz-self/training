...
public final class BRANCH_LABEL extends IsaPrimitive {
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("%s", target.access().bitField(17, 2).load().subtract(current.access().bitField(17, 2).load()).subtract(Data.valueOf(Type.INT(16), 0x1)).toString());
  }
  ...
  public Location access(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    return annotate(new Location(target.access().bitField(17, 2).load().subtract(current.access().bitField(17, 2).load()).subtract(Data.valueOf(Type.INT(16), 0x1))), vars__);
  }
}