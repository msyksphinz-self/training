...
public final class xori extends IsaPrimitive {
  ...
  public xori(final REG rt, final REG rs, final Immediate imm) {
  ...
  public String syntax(final TemporaryVariables tempVars) {
    ...
    return String.format("xori %s, %s, 0x%x", rt.text(vars__), rs.text(vars__), imm.access().load().bigIntegerValue(false));
  }

  @Override
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    return String.format("001110%s%s%s", rs.image(vars__), rt.image(vars__), imm.access().load().toString());
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    final TempVars vars__ = (TempVars) tempVars;
    rt.access(pe__, vars__).store(rs.access(pe__, vars__).load().xor(Data.zeroExtend(WORD, imm.access().load())));
  }
}