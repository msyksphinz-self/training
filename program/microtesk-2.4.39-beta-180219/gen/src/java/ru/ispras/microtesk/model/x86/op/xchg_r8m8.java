...
public final class xchg_r8m8 extends IsaPrimitive {
  private static final class Info extends IsaPrimitiveInfoAnd {
    ...
    public IsaPrimitive create(final Map<String, IsaPrimitive> args) {
      ...
      return new xchg_r8m8(seg_ovrd, dst, src);
    }
  }
  ...
  public xchg_r8m8(final RSEG16 seg_ovrd, final R8 dst, final IsaPrimitive src) {
  ...
  public String image(final TemporaryVariables tempVars) {
    final TempVars vars__ = (TempVars) tempVars;
    init(vars__);
    return String.format("001%s11010000110%s%s%s%s", seg_ovrd.image(vars__), vars__.mod.access().load().toString(), dst.image(vars__), vars__.rm.access().load().toString(), src.image(vars__));
  }
  ...
  public void action(final ProcessingElement procElem, final TemporaryVariables tempVars) {
    ...
    dst.access(pe__, vars__).store(dst.access(pe__, vars__).load().xor(pe__.MEM.access(src.access(pe__, vars__).load()).load()));
    pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load())).store(pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load())).load().xor(dst.access(pe__, vars__).load()));
    dst.access(pe__, vars__).store(dst.access(pe__, vars__).load().xor(pe__.MEM.access(vars__.base.access().load().add(src.access(pe__, vars__).load())).load()));
