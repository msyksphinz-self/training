...
// Global variables needed in update routine
struct DataBlock {
...
__global__ void copy_const_kernel (float *iptr,
                                   const float *cptr)
{
    // Mpping of threadIdx/blockIdx into Pixel
...
__global__ void blend_kernel (float *outSrc,
                              const float *inSrc)
{
    // Mapping of threadIxd/blockIdx into Pixel
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    int offset  = x + y * blockDim.x * gridDim.x;
...
int main (void)
{
    ...
    HANDLE_ERROR (cudaMalloc ((void **)&data.output_bitmap,
                              bitmap.image_size()));
    // Assume that size of float is char x 4
